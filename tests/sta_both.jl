...
staget = zeros(numsta,2); eveid = Vector{Int}()
for i = 1:numeve
    ...
    local eveget = 0; local stajudge = zeros(numsta,2)
    ...
    for j = 1:numpick
        ...
        if picks[j,6] == "S" && picks[j,5] < config["s_requirement"]
            stajudge[sta_id,2] = 1
        end
    end

    for i = 1:numsta
        if stajudge[i,1] == 1 && stajudge[i,2] == 1
            eveget += 1
            staget[i] += 1
        end
    end

    if eveget < config["picks"]
        continue
    end
...
# JSON.print(file2, dic_filter); close(file2)
...
# figure()
# scatter(alleve.y, alleve.x, label = "events")
# scatter(allsta.y, allsta.x, label = "stations")
# legend()
